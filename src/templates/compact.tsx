        ...
        {((content as WorkExperienceContent).items || []).map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 text-[10px] text-zinc-400">{item.startDate} â€“ {item.endDate || (item.current ? (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present') : '')}</span>
            </div>
            {item.description && <p className="mt-0.5 text-xs text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
            {item.technologies?.length > 0 && (
              <p className="mt-0.5 text-[10px] text-zinc-400">{resume.language === 'zh' ? 'æŠ€æœ¯æ ˆ' : 'Tech'}: {item.technologies.join(', ')}</p>
            )}
          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.institution && <span className="text-xs text-zinc-500"> â€” {item.institution}</span>}
                {item.location && <span className="text-xs text-zinc-400">, {item.location}</span>}
              </div>
              <span className="shrink-0 text-[10px] text-zinc-400">{item.startDate} â€“ {item.endDate || (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present')}</span>
            </div>
        ...
        {((content as ProjectsContent).items || []).map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              {item.startDate && <span className="shrink-0 text-[10px] text-zinc-400">{item.startDate} â€“ {item.endDate || (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present')}</span>}
            </div>
            {item.description && <p className="mt-0.5 text-xs text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
            {item.technologies?.length > 0 && (
              <p className="mt-0.5 text-[10px] text-zinc-400">{resume.language === 'zh' ? 'æŠ€æœ¯æ ˆ' : 'Tech'}: {item.technologies.join(', ')}</p>
