        ...
        {((content as WorkExperienceContent).items || []).map((item: any) => (
          <div key={item.id} className="rounded-sm p-3" style={{ backgroundColor: SLATE_50 }}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 text-xs font-light" style={{ color: SLATE_400 }}>{item.startDate} - {item.endDate || (item.current ? (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present') : '')}</span>
            </div>
            ...
            {item.technologies?.length > 0 && (
              <p className="mt-0.5 text-xs font-light" style={{ color: SLATE_400 }}>{resume.language === 'zh' ? 'æŠ€æœ¯æ ˆ' : 'Tech'}: {item.technologies.join(', ')}</p>
            )}
        ...
        {((content as EducationContent).items || []).map((item: any) => (
          <div key={item.id} className="rounded-sm p-3" style={{ backgroundColor: SLATE_50 }}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 text-xs font-light" style={{ color: SLATE_400 }}>{item.startDate} - {item.endDate || (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present')}</span>
            </div>
    ...
    return (
      <div className="space-y-1.5">
        {((content as CertificationsContent).items || []).map((item: any) => (
          <div key={item.id}>
            <span className="text-sm font-medium" style={{ color: SLATE_500 }}>{item.name}</span>
            {(item.issuer || item.date) && <span className="text-sm font-light" style={{ color: SLATE_400 }}>{item.issuer && <> â€” {item.issuer}</>}{item.date && <> ({item.date})</>}</span>}
          </div>
        ))}
      </div>
    );
      ...
      <div className="space-y-1.5">
        {((content as LanguagesContent).items || []).map((item: any) => (
          <div key={item.id}>
            ...
            <span className="text-sm font-light" style={{ color: SLATE_400 }}> â€” {item.proficiency}</span>
          </div>
        ))}
      </div>
    );
      ...
      <div className="space-y-3">
        {items.map((item: any) => (
          <div key={item.id} className="rounded-sm p-3" style={{ backgroundColor: SLATE_50 }}>
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-medium" style={{ color: SLATE_500 }}>{item.name}</span>
              <span className="text-xs font-light" style={{ color: SLATE_400 }}>â­ {item.stars?.toLocaleString()}</span>
            </div>
          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.subtitle && <span className="text-sm font-light" style={{ color: SLATE_400 }}> â€” {item.subtitle}</span>}
              </div>
