        ...
        {((content as WorkExperienceContent).items || []).map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 text-xs font-medium" style={{ color: BLUE_600 }}>{item.startDate} - {item.endDate || (item.current ? (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present') : '')}</span>
            </div>
        ...
        {((content as EducationContent).items || []).map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 text-xs font-medium" style={{ color: BLUE_600 }}>{item.startDate} - {item.endDate || (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present')}</span>
            </div>
    ...
    return (
      <div className="space-y-1.5">
        {((content as CertificationsContent).items || []).map((item: any) => (
          <div key={item.id}>
            <span className="text-sm font-bold" style={{ color: GRAY_700 }}>{item.name}</span>
            {(item.issuer || item.date) && <span className="text-sm text-gray-500">{item.issuer && <> â€” {item.issuer}</>}{item.date && <> ({item.date})</>}</span>}
          </div>
        ))}
      </div>
    );
      ...
      <div className="space-y-1.5">
        {((content as LanguagesContent).items || []).map((item: any) => (
          <div key={item.id}>
            ...
            <span className="text-sm text-gray-500"> â€” {item.proficiency}</span>
          </div>
        ))}
      </div>
    );
      ...
      <div className="space-y-3">
        {items.map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-bold" style={{ color: GRAY_700 }}>{item.name}</span>
              <span className="text-xs text-gray-400">â­ {item.stars?.toLocaleString()}</span>
            </div>
