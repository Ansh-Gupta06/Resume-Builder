          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.company && <span className="text-sm text-zinc-500"> â€” {item.company}</span>}
              </div>
              <span className="shrink-0 text-xs italic text-zinc-400">{item.startDate} â€“ {item.endDate || (item.current ? (lang === 'zh' ? 'è‡³ä»Š' : 'Present') : '')}</span>
            </div>
            {item.description && <p className="mt-1 text-sm text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
            {item.technologies?.length > 0 && <p className="mt-0.5 text-xs text-zinc-400 italic">{lang === 'zh' ? 'æŠ€æœ¯æ ˆ' : 'Tech'}: {item.technologies.join(', ')}</p>}
          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.institution && <span className="text-sm text-zinc-500"> â€” {item.institution}</span>}
              </div>
              <span className="shrink-0 text-xs italic text-zinc-400">{item.startDate} â€“ {item.endDate || (lang === 'zh' ? 'è‡³ä»Š' : 'Present')}</span>
            </div>
        ...
        {items.map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              {item.startDate && <span className="shrink-0 text-xs italic text-zinc-400">{item.startDate} â€“ {item.endDate || (lang === 'zh' ? 'è‡³ä»Š' : 'Present')}</span>}
            </div>
            {item.description && <p className="mt-1 text-sm text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
            {item.technologies?.length > 0 && <p className="mt-0.5 text-xs text-zinc-400 italic">{lang === 'zh' ? 'æŠ€æœ¯æ ˆ' : 'Tech'}: {item.technologies.join(', ')}</p>}
            {item.highlights?.length > 0 && (
        ...
        {items.map((item: any) => (
          <div key={item.id} className="flex items-baseline justify-between text-sm">
            <div>
              ...
              {item.issuer && <span className="text-zinc-500"> â€” {item.issuer}</span>}
            </div>
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-1">
        {items.map((item: any) => (
          <span key={item.id} className="text-sm">
            ...
            <span className="text-zinc-500"> â€” {item.proficiency}</span>
          </span>
        ))}
      </div>
    );
          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.subtitle && <span className="text-sm text-zinc-500"> â€” {item.subtitle}</span>}
              </div>
              {item.date && <span className="shrink-0 text-xs italic text-zinc-400">{item.date}</span>}
            </div>
