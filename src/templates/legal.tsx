        ...
        {((content as WorkExperienceContent).items || []).map((item: any) => (
          <div key={item.id}>
            ...
            {item.technologies?.length > 0 && (
              <p className="mt-0.5 text-xs italic" style={{ color: MUTED }}>{resume.language === 'zh' ? 'æŠ€æœ¯æ ˆ' : 'Technologies'}: {item.technologies.join(', ')}</p>
            )}
      ...
      <div className="space-y-1.5">
        {((content as CertificationsContent).items || []).map((item: any) => (
          <div key={item.id} className="text-sm">
            ...
            {(item.issuer || item.date) && <span style={{ color: MUTED }}>{item.issuer && <> â€” {item.issuer}</>}{item.date && <>, {item.date}</>}</span>}
          </div>
        ))}
      </div>
    );
      ...
      <div className="space-y-3">
        {items.map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-bold" style={{ color: PRIMARY }}>{item.name}</span>
              <span className="shrink-0 text-xs italic" style={{ color: MUTED }}>â­ {item.stars?.toLocaleString()}</span>
            </div>
            {item.language && (
