          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.company && <span className="text-sm text-zinc-600"> â€” {item.company}</span>}
                {item.location && <span className="text-sm text-zinc-400"> ({item.location})</span>}
              </div>
              <span className="shrink-0 text-xs text-zinc-400 italic">{item.startDate} â€“ {item.endDate || (item.current ? (lang === 'zh' ? 'è‡³ä»Š' : 'Present') : '')}</span>
            </div>
        ...
        {items.map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 text-xs text-zinc-400 italic">{item.startDate} â€“ {item.endDate || (lang === 'zh' ? 'è‡³ä»Š' : 'Present')}</span>
            </div>
            ...
            <div className="flex items-baseline justify-between">
              ...
              {item.startDate && (
                <span className="shrink-0 text-xs text-zinc-400 italic">
                  {item.startDate} â€“ {item.endDate || (lang === 'zh' ? 'è‡³ä»Š' : 'Present')}
                </span>
              )}
            </div>
            {item.description && <p className="mt-1 text-sm text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
