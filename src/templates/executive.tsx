          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 rounded px-2 py-0.5 text-xs font-medium text-white" style={{ background: EMERALD }}>
                {item.startDate} â€“ {item.endDate || (item.current ? (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present') : '')}
              </span>
            </div>
          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.institution && <span className="text-sm text-zinc-600"> â€” {item.institution}</span>}
              </div>
        ...
        {items.map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              {item.startDate && (
                <span className="shrink-0 rounded px-2 py-0.5 text-xs font-medium text-white" style={{ background: EMERALD }}>
                  {item.startDate} â€“ {item.endDate || (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present')}
                </span>
              )}
            </div>
            {item.description && <p className="mt-1 text-sm text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
        ...
        {items.map((item: any) => (
          <div key={item.id} className="flex items-baseline justify-between text-sm">
            <div>
              ...
              {item.issuer && <span className="text-zinc-600"> â€” {item.issuer}</span>}
            </div>
      ...
      <div className="flex flex-wrap gap-2">
        {items.map((item: any) => (
          <span key={item.id} className="rounded border px-2.5 py-1 text-xs font-medium" style={{ borderColor: EMERALD, color: CHARCOAL }}>
            {item.language}<span className="text-zinc-500"> â€” {item.proficiency}</span>
          </span>
        ))}
      </div>
    );
