          ...
          <div key={item.id} className="rounded-lg border-l-3 p-3" style={{ borderColor: ACCENT, backgroundColor: WARM_BG }}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.institution && <span className="text-sm" style={{ color: MUTED }}> â€” {item.institution}</span>}
              </div>
      ...
      <div className="space-y-1.5">
        {((content as CertificationsContent).items || []).map((item: any) => (
          <div key={item.id} className="flex items-center gap-2">
            ...
            {(item.issuer || item.date) && <span className="text-sm" style={{ color: MUTED }}>{item.issuer && <> â€” {item.issuer}</>}{item.date && <> ({item.date})</>}</span>}
          </div>
        ))}
      </div>
    );
      ...
      <div className="flex flex-wrap gap-2">
        {((content as LanguagesContent).items || []).map((item: any) => (
          <div key={item.id} className="rounded-full px-3 py-1 text-sm" style={{ backgroundColor: WARM_BG }}>
            ...
            <span style={{ color: MUTED }}> â€” {item.proficiency}</span>
          </div>
        ))}
      </div>
    );
          ...
          <div key={item.id} className="rounded-lg border-l-3 p-3" style={{ borderColor: ACCENT, backgroundColor: WARM_BG }}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.subtitle && <span className="text-sm" style={{ color: MUTED }}> â€” {item.subtitle}</span>}
              </div>
