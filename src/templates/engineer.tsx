          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.institution && <span className="text-sm" style={{ color: SECONDARY }}> â€” {item.institution}</span>}
              </div>
              <span
      ...
      <div className="space-y-3">
        {((content as GitHubContent).items || []).map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-bold" style={{ color: PRIMARY }}>{item.name}</span>
              <span className="shrink-0 text-xs" style={{ fontFamily: 'JetBrains Mono, Consolas, monospace', color: SECONDARY }}>â­ {item.stars?.toLocaleString()}</span>
            </div>
      ...
      <div className="space-y-1.5">
        {((content as CertificationsContent).items || []).map((item: any) => (
          <div key={item.id} className="flex items-center gap-2">
            ...
            {(item.issuer || item.date) && <span className="text-sm" style={{ color: SECONDARY }}>{item.issuer && <> â€” {item.issuer}</>}{item.date && <> ({item.date})</>}</span>}
          </div>
        ))}
      </div>
    );
      ...
      <div className="space-y-1.5">
        {((content as LanguagesContent).items || []).map((item: any) => (
          <div key={item.id} className="flex items-center gap-2">
            ...
            <span className="text-sm" style={{ color: SECONDARY }}> â€” {item.proficiency}</span>
          </div>
        ))}
      </div>
    );
          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.subtitle && <span className="text-sm" style={{ color: SECONDARY }}> â€” {item.subtitle}</span>}
              </div>
              {item.date && (
                <span
                  className="shrink-0 text-xs"
      ...
      <div className="space-y-2">
        {content.items.map((item: any) => (
          <div key={item.id} className="flex items-center gap-2">
            ...
            {item.description && <span className="text-sm" style={{ color: BODY_TEXT }} dangerouslySetInnerHTML={{ __html: ' â€” ' + md(item.description) }} />}
          </div>
        ))}
      </div>
    );
