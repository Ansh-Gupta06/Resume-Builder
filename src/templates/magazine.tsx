        ...
        {items.map((item: any) => (
          <div key={item.id} className="border-l-2 pl-4" style={{ borderColor: ACCENT }}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-xs" style={{ color: SECONDARY }}>{item.startDate} - {item.endDate || (resume.language === 'zh' ? 'è‡³ä»Š' : 'Present')}</span>
            </div>
            <p className="text-sm" style={{ color: SECONDARY }}>{degreeField(item.degree, item.field)}{item.location ? ` â€” ${item.location}` : ''}</p>
