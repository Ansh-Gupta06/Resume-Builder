...
import { isSectionEmpty, md, degreeField } from './render-utils';
        ...
        {items.map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-xs text-zinc-400">{item.startDate} - {item.endDate || (item.current ? (lang === 'zh' ? 'è‡³ä»Š' : 'Present') : '')}</span>
            </div>
            {item.description && <p className="mt-1 text-sm text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
            {item.technologies?.length > 0 && (
              <p className="mt-0.5 text-xs text-zinc-400">{lang === 'zh' ? 'æŠ€æœ¯æ ˆ' : 'Tech'}: {item.technologies.join(', ')}</p>
            )}
        ...
        {items.map((item: any) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="text-xs text-zinc-400">{item.startDate} - {item.endDate || (lang === 'zh' ? 'è‡³ä»Š' : 'Present')}</span>
            </div>
            ...
            <div className="flex items-baseline justify-between">
              ...
              {item.startDate && (
                <span className="text-xs text-zinc-400">
                  {item.startDate} - {item.endDate || (lang === 'zh' ? 'è‡³ä»Š' : 'Present')}
                </span>
              )}
            </div>
            {item.description && <p className="mt-1 text-sm text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
    ...
    return (
      <div className="space-y-1">
        {items.map((item: any) => (
          <div key={item.id}>
            <span className="font-semibold text-zinc-800 text-sm">{item.name}</span>
            {(item.issuer || item.date) && <span className="text-sm text-zinc-600">{item.issuer && <> â€” {item.issuer}</>}{item.date && <> ({item.date})</>}</span>}
          </div>
        ))}
      </div>
    );
      ...
      <div className="space-y-1">
        {items.map((item: any) => (
          <div key={item.id}>
            ...
            <span className="text-sm text-zinc-600"> â€” {item.proficiency}</span>
          </div>
        ))}
      </div>
    );
